...
@section('page_content')
    ...
    <!-- Category Tree Modal -->
    <div class="modal fade" id="categoryTree" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle"
         aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Kategori Ağacı</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" id="categoryTreeBody">
                </div>
...
@section('page_scripts')
    <script type="text/javascript">
        function getCategoryTree() {
            $('#categoryTreeBody').html('');
            $('#up_category_id').val();
            var categoryTreeBody = '';
            $.ajaxSetup({
                headers: {
            ...
            $.ajax({
                ...
                beforeSend: function () {
                    $('#categoryTreeBody').html('Yükleniyor...');
                },
                error: function (data) {
    ...
    <script type="text/javascript">
        ...
        function detail(id) {
            ...
            $("#editItemForm #messages").html(null);
            $('#editItemForm #title_current').val('');
            $('#editItemForm #title').val('');
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
